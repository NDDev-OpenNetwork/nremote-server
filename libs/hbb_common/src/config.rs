...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("NRemote".to_owned());
...
pub const LINK_DOCS_X11_REQUIRED: &str =
    "https://github.com/NDDev-OpenNetwork/nremote#x11-required";
pub const LINK_HEADLESS_LINUX_SUPPORT: &str =
    "https://github.com/NDDev-OpenNetwork/nremote#headless-linux";

lazy_static::lazy_static! {
    pub static ref HELPER_URL: HashMap<&'static str, &'static str> = HashMap::from([
        ("nremote docs home", LINK_DOCS_HOME),
        ("nremote docs x11-required", LINK_DOCS_X11_REQUIRED),
        ("nremote x11 headless", LINK_HEADLESS_LINUX_SUPPORT),
        ]);
}
...
// No third-party default. A build that ships one silently sends every
// unconfigured client to somebody else's infrastructure, which is the
// situation self-hosting exists to end. The address is supplied at build
// time or configured by the operator; absent both, there is none.
pub const RENDEZVOUS_SERVERS: &[&str] = &[];
// Paired with RENDEZVOUS_SERVERS above: no default server, so no default key.
